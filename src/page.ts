...
export function renderCenterFuseHome(urls: ProductUrls = resolveProductUrls({})): string {
  const sellAction = urls.SELLFUSE ? `<a class="button" href="${urls.SELLFUSE}">Start with SellFuse</a>` : `<span class="button secondary" aria-disabled="true">SellFuse protected</span>`;
  const buyAction = urls.BUYFUSE ? `<a class="button outline" href="${urls.BUYFUSE}">Open BuyFuse</a>` : `<span class="button secondary" aria-disabled="true">BuyFuse unavailable</span>`;
  const sellCardAction = urls.SELLFUSE ? `<a class="button" href="${urls.SELLFUSE}">Prepare a listing</a>` : `<p class="state">The seller surface is protected and is not connected to this public host.</p>`;
  const buyCardAction = urls.BUYFUSE ? `<a class="button" href="${urls.BUYFUSE}">Organize items</a>` : `<p class="state">The buyer surface is not currently connected to this host.</p>`;
  const body = `${renderFamilyHeader("CENTERFUSE", urls)}<main id="main">
  <section class="shell hero"><p class="eyebrow">One account. A growing family of useful tools.</p><h1 class="display">Bring buying and selling into focus.</h1><p class="lede">CenterFuse is the home of focused products that reduce the busywork around everyday commerce.</p><div class="row">${sellAction}${buyAction}</div></section>
  <section class="shell" aria-labelledby="products"><p class="eyebrow">Products</p><h2 id="products">Choose the tool for what you are doing today.</h2><div class="grid">
    <article class="card"><p class="eyebrow">For sellers</p><h2>SellFuse</h2><p>Turn photos and what you know into a reviewed master listing, then prepare it for the places you choose to sell.</p>${sellCardAction}</article>
    <article class="card"><p class="eyebrow">For buyers</p><h2>BuyFuse</h2><p>Keep items you are considering, their source links, and the details you want to remember in one calm workspace.</p>${buyCardAction}</article>
  </div></section>
